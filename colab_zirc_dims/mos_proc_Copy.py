...
def load_data_dict(ROOT_DIR_STRING):
    ...
    # save_path = \
    # os.path.join(ROOT_DIR, 'training_images') #MODIFY FOR FINAL VERSION

    # loads info csv as dictionary
    mos_csv_dict = pd.read_csv(
        mos_csv_path, header=0, index_col=False, squeeze=False
    ).to_dict("list")
    ...
    if all(key in mos_csv_dict_keys for key in req_keys):
        print("Incorrect mosaic_info.csv headers: correct and re-save")
        return {}

    # lists of files in directories
    mosaic_bmp_filenames = [
        file for file in os.listdir(mosaic_path) if file.endswith(".bmp")
    ]
    mosaic_align_filenames = [
        file for file in os.listdir(mosaic_path) if file.endswith(".Align")
    ]
    ...
    # loops through mos_csv_dict in order to collect data, verify \
    # that all files given in mosaic_info.csv are present
    for eachindex, eachsample in enumerate(mos_csv_dict["Sample"]):
        ...
        each_csv_mosaic_name = mos_csv_dict["Mosaic"][eachindex].split(".")[
            0
        ...
        if any(each_csv_scanlist_name in string for string in scanlist_filenames):
            temp_scn_file = [
                string
                for string in scanlist_filenames
                if each_csv_scanlist_name in string
            ][0]
            ...
            if any(each_csv_mosaic_name in string for string in mosaic_bmp_filenames):
                temp_mos_file = [
                    string
                    for string in mosaic_bmp_filenames
                    if each_csv_mosaic_name in string
                ][0]
                act_mos_file = os.path.join(mosaic_path, temp_mos_file)

                if any(
                    each_csv_mosaic_name in string for string in mosaic_align_filenames
                ):

                    temp_align_file = [
                        string
                        for string in mosaic_align_filenames
                        if each_csv_mosaic_name in string
                    ][0]
        ...
        if each_include_bool:
            ...
            # list of scans added to output dictionary
            added_scans_unchanged = []

            # loops through shotlist and gets coordinates \
            # for each scan + numbers repeated instances
            for eachscan_index, eachscan in enumerate(each_scanlist["Description"]):
                if each_scanlist["Scan Type"][eachscan_index] == "Spot":
                    eachscan_coords = [
                        float(data)
                        for data in each_scanlist["Vertex List"][eachscan_index].split(
                            ","
                        )
                    ][:2]
                    if eachscan in added_scans_unchanged:
                        temp_scanname = (
                            str(eachscan)
                            + "-"
                            + str(added_scans_unchanged.count(eachscan) + 1)
                        )
            ...
            temp_output_dict[eachsample]["Max_zircon_size"] = mos_csv_dict[
                "Max_zircon_size"
            ][eachindex]
            temp_output_dict[eachsample]["Offsets"] = [
                mos_csv_dict["X_offset"][eachindex],
...
# function for retrieving mask at center of image, \
# ~nearest to center of image
def get_central_mask(results):

    # class for generating points, moving ~radially outwards from  \
    # center of image, to find central mask if not at actual center
    class point_generator:
        def __init__(self, x_center, y_center, pixel_increment):
            self.x_center, self.y_center, self.pixel_increment = (
                x_center,
                y_center,
                pixel_increment,
            )
            ...
            # int from 0-7 whether x or y are added to or \
            # subtracted from; at 0 x is added to.
            self.rot_counter = 0
        ...
        def update_pts(self):
            ...
            if self.rot_counter == 4:
                self.curr_pts = [
                    int(self.x_center + self.curr_margin),
                    int(self.y_center - self.curr_margin),
                ]
    ...
    masks = np.stack(
        [
            eachresult.cpu().numpy()
            for eachresult in results["instances"].get("pred_masks")
        ],
        2,
    )
    ...
    while (not mask_found_bool) and (pts.inc_multiplier < 20):
        pts.next_inc()
...
# measures zircon mask (in pixels) using skimage, creates an \
# image by overlaying the mask atop the original subimage, \
# and optionally displays (if display_bool = True) and/or \
# saves (if save_dir != '') this image.
def overlay_mask_and_get_props(
    input_central_mask,
    original_image,
    analys_name,
    display_bool=False,
    ...
    # selects largest region in case central zircon \
    # mask has multiple disconnected regions
...
# a class for a mosaic image from which subimages \
# are clipped, with functions for clipping
class mos_img:
    ...
    def __init__(
        ...
        if skimage.exposure.is_low_contrast(self.orig_full_img, 0.10):
            ...
            # code for enhancing ctrst by channel: avoids warning but produces grainy images:
            ...
            # self.full_img = skimage.img_as_ubyte(
                # np.stack([self.red, self.green, self.blue], axis=2)
            # )

            # code for enhancing contrast without splitting channels: results are less grainy
            self.full_img = skimage.img_as_ubyte(
                skimage.exposure.equalize_hist(self.orig_full_img)
            )
        ...
        if align_file_path != "":
            ...
            self.align_x_size, self.align_y_size = self.mos_dims[
                1:
            ]  # image dimensions in microns from align file

            # image center in microns from align file or (default)\
            # of image (in pixels) from image dimensions
            self.align_x_center, self.align_y_center = (
                self.align_x_size // 2,
                self.align_y_size // 2,
            )
                ...
                if eachchild.tag == "Alignment":
                    for each_align_data in eachchild:
                        if each_align_data.tag == "Center":
                            self.align_x_center, self.align_y_center = [
                                float(data) for data in each_align_data.text.split(",")
                            ]
                        if each_align_data.tag == "Size":
                            self.align_x_size, self.align_y_size = [
                                float(data) for data in each_align_data.text.split(",")
                            ]

            # sets new origin based on data in .Align file
            self.x_origin = int(
                self.align_x_center - self.align_x_size / 2 - x_y_shift_list[0]
            )
            self.y_origin = int(
                self.align_y_center - self.align_y_size / 2 - x_y_shift_list[1]
            )

            # sets new scale factor (microns(?)/pixel) based on \
            # dimensions of mosaic image and those in .Align file
            self.scale_factor = (
                self.align_x_size / self.mos_dims[1]
                + self.align_y_size / self.mos_dims[2]
            ) / 2
        ...
        self.sub_img_size = int(
            sub_img_size / self.scale_factor
        )  # number of pixels for subimages from micron input
    ...
    def coords_to_pix(self, x_coord, y_coord):
        ...
        return (
            (x_coord - self.x_origin) / self.scale_factor,
            (y_coord - self.y_origin) / self.scale_factor,
        )

    def set_subimg(
        self, x_coord, y_coord
    ):  # x_coord and y_coord (um) are the center of the new subimage generated

        # initializes variables for extent of desired subimage \
        # beyond bounds of mosaic image; these will ensure that the
        # central point of subimage is that of input coordinates. \
        # Subimage pixels beyond mosaic image bounds will be black.
        ...
        # sets initial clipping bounds (in pixels) for a subimage \
        # of size sub_img_size^2 centered at input coordinates
        self.x0 = int(
            (x_coord - self.x_origin) / self.scale_factor - self.sub_img_size / 2
        )
        self.x1 = int(
            (x_coord - self.x_origin) / self.scale_factor + self.sub_img_size / 2
        )
        self.y0 = int(
            (y_coord - self.y_origin) / self.scale_factor - self.sub_img_size / 2
        )
        self.y1 = int(
            (y_coord - self.y_origin) / self.scale_factor + self.sub_img_size / 2
        )

        # modifies coordinates in the case that \
        # initial bounds exceed image bounds
        if self.x0 < 0:
            # print('x0 adjusted')
        ...
        if self.x1 < 0:
            # in case of points that are fully out of bounds, \
            # a fully black subimage will be displayed
            self.x1 = 0
        ...
        self.sub_img = np.zeros(
            [self.sub_img_size, self.sub_img_size, self.mos_dims[0]], dtype=np.uint8
        )

        # maps crop from mosaic onto black image to create \
        # subimage with input coordinate point at center
        self.sub_img[
            self.y0_offset:self.y1_offset, self.x0_offset:self.x1_offset, :
        ] = self.full_img[self.y0:self.y1, self.x0:self.x1, :]
